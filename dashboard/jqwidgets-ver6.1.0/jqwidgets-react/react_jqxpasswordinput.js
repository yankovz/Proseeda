...
export default class JqxPasswordInput extends React.Component {
    constructor(props) {
        super(props);
        const widgetId = 'jqxPasswordInput' + JQXLite.generateID();
    ...
    manageAttributes() {
        const properties = ['disabled','height','localization','maxLength','placeHolder','passwordStrength','rtl','strengthColors','showStrength','showStrengthPosition','strengthTypeRenderer','showPasswordIcon','theme','width'];
        let options = {};
    ...
    render() {
        return (
            <input type='password' id={this.state.id}></input>
        )
    };
};
