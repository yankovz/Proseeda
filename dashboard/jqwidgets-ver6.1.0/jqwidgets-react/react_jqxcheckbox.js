...
export default class JqxCheckBox extends React.Component {
    constructor(props) {
        super(props);
        const widgetId = 'jqxCheckBox' + JQXLite.generateID();
    ...
    manageAttributes() {
        const properties = ['animationShowDelay','animationHideDelay','boxSize','checked','disabled','enableContainerClick','groupName','height','hasThreeStates','locked','rtl','theme','width'];
        let options = {};
    ...
    render() {
        return (
            <div id={this.state.id}><span>{this.props.value}</span></div>
        )
    };
};
